...
def lambda_handler(event, handler):
    ...
    for obj in src_bucket.objects.filter(Prefix=file_pickup_folder):
        ...
        for line in lines:
            ...
            # condition for bad error record
                ...
                error_file_400 += line + "::" + str(
                    response.status_code) + "::" + response.reason + "::" + current_timestamp_str + "::" + key + "\n"
            ...
            # condition for unprocessed record due to API error
                ...
                current_timestamp_str = current_timestamp.strftime('%Y-%m-%dT%H:%M:%S') + (
                            '-%02d' % (current_timestamp.microsecond / 10000))
                error_file_other_than_400 += line + "::" + str(
                    response.status_code) + "::" + response.reason + "::" + current_timestamp_str + "::" + key + "\n"

        logger.info(
            "Successful records count is > " + str(count) + ", Total record count is -> " + str(no_of_lines_in_file))
    ...
    if len(list_error_400) > 1:
        ...
        create_file_in_s3(error_file_400, error_file_name, error_bucket_name, s3)
    ...
    if len(list_error_other_than_400) > 1:
        ...
        create_file_in_s3(error_file_other_than_400, error_file_name, error_bucket_name, s3)
...
def create_file_in_s3(data, file_name, bucket_name, s3):
    ...
    s3.Bucket(bucket_name).put_object(
        Key="ErrorLogFiles/" + str(year) + "/" + str(month) + "/" + str(day) + "/" + file_name, Body=encoded_string)

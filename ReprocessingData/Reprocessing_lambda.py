    ...
    for obj in src_bucket.objects.filter(Prefix="ErrorLogFiles/"+str(year)+"/"+str(month)+"/"+str(day)+"/"):
        ...
        for i in df.index:
            ...
            if response.status_code >=200 and response.status_code<300:
                ...
                success_list.append([line,str(response.status_code),response.reason,current_timestamp_str,df['File Name'].iloc[i]])
        ...
        #         count += 1
                ...
                error400_list.append([line,str(response.status_code),response.reason,current_timestamp_str,df['File Name'].iloc[i]])
            # condition for unprocessed eror
            else:
                current_timestamp = datetime.datetime.now()
                current_timestamp_str=current_timestamp.strftime('%Y-%m-%dT%H:%M:%S') + ('-%02d' % (current_timestamp.microsecond / 10000))
                
                error_other_than_400_list.append([line,str(response.status_code),response.reason,current_timestamp_str,df['File Name'].iloc[i]])
        ...
        if len(error_other_than_400_list)>1:
            ...
            current_timestamp = datetime.datetime.now()
            current_timestamp_str=current_timestamp.strftime('%Y-%m-%dT%H:%M:%S') + ('-%02d' % (current_timestamp.microsecond / 10000))
            create_file_in_s3(data_string, key.replace(".log","")+"_attempted_at_"+current_timestamp_str+".log",error_bucket_name,s3)
